...
***** Quiz ******
A simple javascript ES6, HTML, CSS3, jquery web page. I have not used classes, state management
or imports, this small app is designed to be as efficient as possible.

***** Sign Form ******
An interactive Sign in form, using javascript ES6, HTML5, CSS3, Jquery.
...
region.Prism framework is loaded in each ViewModel using the prism base classes which exposes 
improved versions of IPropertyNotifyChange interface, Prism' own commanding pattern and much more!
...
is a small mobile app which retrieves weather data from a rest API, deserialised from json and then
leveraged into a lightweight database object (Sqlite) which can be programmed against c#.
...
Commanding base classe and the use of action delegates. I have used the standard MVVM parttern to decouple
...
data into a database, then retrieves the data and then is displayed in the UI.
The application makes use of the MVVM pattern, SQL Lite, LINQ to SQL, Commanding, 
IPropertyNotifyChanged, Custom renderers, behaviours, Property binding in XAML, resources, 
Dependency properties, dependency injection, and asynchronous programming.
There are faults within this application which is an ongoing unfinished project.
